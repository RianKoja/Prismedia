...
## [Unreleased]
### What's New
- Books are now more than comics. EPUB and PDF books are scanned as first-class library items — their cover, title, and author are read straight from the file (PDF covers are rendered from the first page) — and open in a built-in reader. It picks the right mode automatically: EPUBs reflow with paged or scrolling layouts and adjustable text size, while PDFs render their fixed pages. A Contents button opens the book's table of contents (or PDF outline) to jump between sections, on-screen page arrows and tap zones (tap the sides to turn pages, the middle to show or hide the toolbar) make navigation obvious on desktop and mobile, in-book links use the brass accent, and the reader remembers exactly where you left off. Comics continue to read exactly as before.
...
- The navigation sidebar is now yours to arrange. An Edit mode (the pencil at the bottom of the rail) lets you rename and reorder sections, create your own sections, drag items between them, and hide the destinations you never use — all saved on your device. Hidden items reappear in Edit mode so you can switch them back on anytime, and Reset restores the original layout.
...
### Added
- Added EPUB and PDF as single-file book formats: `.epub` and `.pdf` files scan into one book entity each (no chapter/page entities), with title/author/description read from the file (VersOne.Epub / PdfPig) and a cover taken from the EPUB's embedded image or the PDF's rendered first page (PDFtoImage). Both open in one reader built on foliate-js — reflowable for EPUB (paged/scrolled, adjustable text size), fixed-page for PDF — with an in-file table of contents/outline popup, comic-style tap zones plus prev/next page buttons, accent-styled links, and per-book resume that streams the source file and stores an opaque location (EPUB CFI) plus progress percent.

...
- Identifying an album now matches its tracks by track number instead of the on-disk filename and renames them to the canonical titles. Files named like "Artist - Album - 03 - Believer" line up with the release's track list by position and become "Believer", so messy or tagless filenames still get clean titles. (This updates the track's title in Prismedia; the file on disk is left untouched.)

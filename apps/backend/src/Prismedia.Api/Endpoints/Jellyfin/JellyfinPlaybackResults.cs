...
internal static class JellyfinPlaybackResults {
    internal static async Task<IResult> GetPlaybackInfoAsync(
        ...
        return info is null
            ? Results.NotFound(new ApiProblem(ApiProblemCodes.PlaybackSourceNotFound, $"Item '{itemId}' has no playable source."))
    ...
    internal static async Task<IResult> StreamVideoAsync(
        ...
        CancellationToken cancellationToken) {
        if (!await IsVisibleAsync(itemId, entities, httpContext, cancellationToken)) {
            return Results.NotFound(new ApiProblem(ApiProblemCodes.VideoStreamNotFound, $"Video stream '{itemId}' was not found."));
        }
    ...
    internal static async Task<IResult> GetTrickplayPlaylistAsync(
        ...
        CancellationToken cancellationToken) {
        if (!await IsVisibleAsync(itemId, entities, httpContext, cancellationToken)) {
            return Results.NotFound(new ApiProblem(ApiProblemCodes.VideoTrickplayNotFound, $"Trickplay width '{width}' for '{itemId}' was not found."));
        }
    ...
    internal static async Task<IResult> GetTrickplayTileAsync(
        ...
        CancellationToken cancellationToken) {
        if (!await IsVisibleAsync(itemId, entities, httpContext, cancellationToken)) {
            return Results.NotFound(new ApiProblem(ApiProblemCodes.VideoTrickplayTileNotFound, $"Trickplay tile '{index}' for '{itemId}' was not found."));
        }

...
internal static partial class PrismediaModelConfiguration {
    private static void ConfigureSystemTables(ModelBuilder modelBuilder) {
        ...
        modelBuilder.Entity<RequestServiceInstanceRow>(entity => {
            entity.ToTable("request_service_instances");
            entity.HasKey(row => row.Id);
            entity.Property(row => row.Id).HasColumnName("id").ValueGeneratedNever();
            ...
            entity.Property(row => row.CreatedAt).HasColumnName("created_at");
            entity.Property(row => row.UpdatedAt).HasColumnName("updated_at");
            entity.HasIndex(row => new { row.Kind, row.IsDefault });
        });

        modelBuilder.Entity<RequestServiceCredentialRow>(entity => {
            entity.ToTable("request_service_credentials");
            entity.HasKey(row => row.Id);
            entity.Property(row => row.Id).HasColumnName("id").ValueGeneratedNever();
            ...
            entity.Property(row => row.CreatedAt).HasColumnName("created_at");
            entity.Property(row => row.UpdatedAt).HasColumnName("updated_at");
            entity.HasIndex(row => new { row.ServiceInstanceId, row.CredentialKey }).IsUnique();

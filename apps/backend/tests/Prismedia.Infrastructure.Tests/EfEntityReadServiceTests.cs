...
public sealed class EfEntityReadServiceTests {
    ...
    [Fact]
    public async Task ListAsyncProjectsAudioTechnicalMetadataAsThumbnailMeta() {
        await using var db = CreateContext();
        ...
        var result = await service.ListAsync(EntityKindRegistry.AudioTrack.Code, null, null, null, null, CancellationToken.None);
        var item = Assert.Single(result.Items);

        Assert.Equal(
            [
                new EntityThumbnailMeta("duration", "03:34"),
                new EntityThumbnailMeta("audio", "44.1 kHz"),
                new EntityThumbnailMeta("audio", "Stereo"),
                new EntityThumbnailMeta("audio", "AAC"),
                new EntityThumbnailMeta("audio", "MP4")
            ],
            item.Meta);
    }

    [Fact]
    public async Task ListAsyncProjectsImageTechnicalMetadataAsThumbnailMeta() {
        await using var db = CreateContext();
        ...
        var result = await service.ListAsync(EntityKindRegistry.Image.Code, null, null, null, null, CancellationToken.None);
        var item = Assert.Single(result.Items);

        Assert.Equal(
            [
                new EntityThumbnailMeta("image", "4000×3000"),
                new EntityThumbnailMeta("image", "JPEG")
            ],
            item.Meta);
    }

    [Fact]
    public async Task ListAsyncProjectsPositionAndStatsAsThumbnailMeta() {
        await using var db = CreateContext();
        var chapterId = Guid.Parse("88888888-8888-8888-8888-888888888888");
        var now = DateTimeOffset.UtcNow;
        db.Entities.Add(new EntityRow {
            Id = chapterId,
        ...
        var result = await service.ListAsync(EntityKindRegistry.BookChapter.Code, null, null, null, null, CancellationToken.None);
        var item = Assert.Single(result.Items);

        Assert.Equal(
            [
                new EntityThumbnailMeta("chapter", "Ch 4"),
                new EntityThumbnailMeta("book", "32 pages")
            ],
            item.Meta);
    }

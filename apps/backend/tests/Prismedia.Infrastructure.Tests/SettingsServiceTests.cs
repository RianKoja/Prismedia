...
public sealed class SettingsServiceTests {
    ...
    [InlineData("23505", "IX_something_else", false)] // right error code, wrong constraint.
    [InlineData("23503", "IX_library_roots_path", false)] // right constraint name, wrong error code.
    [InlineData(null, null, false)]
    public void IsLibraryRootPathConstraintViolationMatchesOnlyThePathUniqueIndex(
        string? sqlState, string? constraintName, bool expected) =>
        Assert.Equal(expected, EfSettingsPersistence.IsLibraryRootPathConstraintViolation(sqlState, constraintName));
